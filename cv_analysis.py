...
for r in input_r:
    ...
    for g in gvariable:
        ...
        # L23--e
        for ii in range(parameters_sim.n_e_23):
            train_23_e = np.load('E:\\Zeldenrust lab\\pythoncodes\\balanced_state\\dataset25\\baseline\\Spiketraintest_23e_g=%g_r=%g.npy'%(g,r),
        ...
        # L23--i
        for ii in range(parameters_sim.n_i_23):
            train_23_i = np.load('E:\\Zeldenrust lab\\pythoncodes\\balanced_state\\dataset25\\baseline\\Spiketraintest_23i_g=%g_r=%g.npy'%(g,r),
                             allow_pickle=True)[0][ii]
        ...
        # L4--e
        for ii in range(parameters_sim.n_e_4):
            train_4_e = np.load('E:\\Zeldenrust lab\\pythoncodes\\balanced_state\\dataset25\\baseline\\Spiketraintest_4e_g=%g_r=%g.npy'%(g,r),
                            allow_pickle=True)[0][ii]
            cv_4_e = CV(train_4_e)
            if cv_4_e != cv_4_e:
                print('nan e')
        ...
        # L4--i
        for ii in range(parameters_sim.n_i_4):
            train_4_i = np.load('E:\\Zeldenrust lab\\pythoncodes\\balanced_state\\dataset25\\baseline\\Spiketraintest_4i_g=%g_r=%g.npy'%(g,r),
                            allow_pickle=True)[0][ii]
            cv_4_i = CV(train_4_i)
            if cv_4_i != cv_4_i:
                print('nan i')
...
f = open('E:\\Zeldenrust lab\\pythoncodes\\balanced_state\\dataset25\\baseline\\cv_values.pckl', 'wb')
